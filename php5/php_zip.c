...
static int php_zip_parse_options(zval *options, long *remove_all_path,
	char **remove_path, int *remove_path_len,
	char **add_path, int *add_path_len,
	long *flags
	TSRMLS_DC) /* {{{ */
{
	zval **option;
...
static long php_zip_status(struct zip *za TSRMLS_DC) /* {{{ */
{
...
static long php_zip_status_sys(struct zip *za TSRMLS_DC) /* {{{ */
{
#if LIBZIP_VERSION_MAJOR < 1
	int zep, syp;
...
static long php_zip_get_num_files(struct zip *za TSRMLS_DC) /* {{{ */
{
...
typedef long (*zip_read_int_t)(struct zip *za TSRMLS_DC);
...
static void php_zip_object_free_storage(void *object TSRMLS_DC) /* {{{ */
{
	...
	if (intern->za) {
		if (zip_close(intern->za) != 0) {
			...
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot destroy the zip context: zip_close have failed");
#else
...
#if LIBZIP_VERSION_MAJOR == 1 && LIBZIP_VERSION_MINOR == 3 && LIBZIP_VERSION_MICRO == 1
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", "zip_close have failed");
#else
...
static void php_zip_add_from_pattern(INTERNAL_FUNCTION_PARAMETERS, int type) /* {{{ */
{
	...
	long glob_flags = 0;
	long zip_flags = ZIP_FL_OVERWRITE;
	zval *options = NULL;
	int found;
	...
	if (options && (php_zip_parse_options(options, &remove_all_path, &remove_path, &remove_path_len,
			&add_path, &add_path_len, &zip_flags TSRMLS_CC) < 0)) {
		RETURN_FALSE;
	}
		...
		for (i = 0; i < found; i++) {
			...
			if (zend_hash_index_find(Z_ARRVAL_P(return_value), i, (void **) &zval_file) == SUCCESS) {
				...
				if (php_zip_add_file(intern, Z_STRVAL_PP(zval_file), Z_STRLEN_PP(zval_file),
					entry_name, entry_name_len, 0, 0, -1, zip_flags TSRMLS_CC) < 0) {
					zval_dtor(return_value);
					RETURN_FALSE;
				}
			}
